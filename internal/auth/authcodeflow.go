...
func reqAuthTknFlow(ClientID, redirectURI, Scope, authCode, userAgent string) error {
    ...
    _, err = parseRespTokens(string(respBody))
...
func InvokeAuthTokenFlow(intuneBypass bool){
	...
	reqAuthTknFlow(clientID, redirURI, fmt.Sprintf("%s %s", scope, resourceURL), authCode, classes.UserAgent)
	fmt.Println()
	// fmt.Println(redeemedHTTPResp)
